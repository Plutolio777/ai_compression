<template>
  ...
  <div class="bg-white p-4 shadow-sm sticky top-16 z-10 w-full">
    <div class="flex items-center justify-between px-8">
      ...
      <div class="flex items-center space-x-4">
        <button
          class="!rounded-button p-2 text-gray-600 hover:bg-gray-100"
  ...
  <div class="flex p-6">
    ...
    <div class="flex-1 p-6">
      ...
      <div v-else class="bg-white rounded-lg shadow-sm">
        <div
          class="grid grid-cols-12 gap-4 p-4 text-sm font-medium text-gray-600 border-b"
        >
    ...
    <div v-if="selectedFiles.length === 1" class="w-80 bg-white shadow-sm p-6 border-l">
      <div class="space-y-6">
        ...
        <div>
          ...
          <div
            class="aspect-video bg-gray-100 rounded-lg flex items-center justify-center relative"
          >
            <div
              v-if="previewLoading"
              ...
              <template v-else>
                ...
                <template v-else-if="selectedFileType === 'video'">
                  <div class="relative w-full h-full">
                    <i
                      class="fas fa-play-circle text-4xl text-gray-400 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
                    ></i>
                    <img
                      :src="selectedFilePreview"
                ...
                <template v-else>
                  <div class="text-center p-4">
                    <i
                      :class="getFileIcon(selectedFileType)"
                      class="text-4xl"
                      :style="{ color: getFileColor(selectedFileType) }"
                    ></i>
        ...
        <div class="flex flex-col space-y-2">
          ...
          <button
            class="!rounded-button w-full px-4 py-2 border border-gray-300 hover:bg-gray-50"
          >
            生成分享链接
          </button>
          <button
          ...
          <button
            class="!rounded-button w-full px-4 py-2 border border-red-300 text-red-500 hover:bg-red-50"
          >
            删除
          </button>
        </div>
  ...
  <div
    v-if="showTagModal"
    class="fixed bg-white shadow-lg rounded-lg z-50"
    :style="{
...
const getCurrentLevelFiles = () => {
  ...
  for (const folderName of currentPath.value) {
    const folder = currentFiles.find(
      (f) => f.type === "folder" && f.name === folderName && Array.isArray(f.children)
    );
...
const closeContextMenu = (event: MouseEvent) => {
  // 不关闭右键菜单如果点击的是标签弹窗
  const isTagModalClick = event
    .composedPath()
    .some((el) => el.classList && el.classList.contains("tag-selector-modal"));
